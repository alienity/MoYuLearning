...
float4 SampleVBuffer(Texture3D VBuffer, SamplerState clampSampler,
                     float2 positionNDC,
                     ...
                     float _VBufferRcpSliceCount)
{
    ...
    if (clampToBorder)
    {
        ...
        coordIsInsideFrustum = max(abs(positionCS.x), max(abs(positionCS.y), abs(positionCS.z))) < 1;
    }
    else
    {
    ...
    if (coordIsInsideFrustum)
    {
        if (quadraticFilterXY)
        {
            ...
            result = (weights[0].x * weights[0].y) * VBuffer.SampleLevel(clampSampler, min(float3(texUv0, texW), VBufferViewportLimit), 0)
                   + (weights[1].x * weights[0].y) * VBuffer.SampleLevel(clampSampler, min(float3(texUv1, texW), VBufferViewportLimit), 0)
                   + (weights[0].x * weights[1].y) * VBuffer.SampleLevel(clampSampler, min(float3(texUv2, texW), VBufferViewportLimit), 0)
                   + (weights[1].x * weights[1].y) * VBuffer.SampleLevel(clampSampler, min(float3(texUv3, texW), VBufferViewportLimit), 0);
        }
        else
        {
            // And these are the texture coordinates.
            ...
            // The sampler clamps to the edge (so UVWs < 0 are OK).
            result = VBuffer.SampleLevel(clampSampler, min(texUVW, VBufferViewportLimit), 0);
        }
    }
                     ...
                     float    _VBufferRcpSliceCount)
{
    ...
    return SampleVBuffer(VBuffer, clampSampler,
                         positionNDC,
                         linearDistance,
                         VBufferViewportSize,
