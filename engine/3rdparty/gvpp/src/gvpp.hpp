...
#define printLineT printLine<chartype>
	...
	private:
		...
		{
			if (lab.length() > 0) (*this)[CWSTR(chartype, "label")] = lab;
		}
		strtype id;
	};
	...
	class Edge : public std::pair<NodeT&, NodeT&>, public ElementT {
		friend class AbstractGraphT;
	public:
		Edge(AbstractGraphT& g, NodeT& n1, NodeT& n2, strtype lab) : std::pair<NodeT&, NodeT&>(n1, n2), ElementT(g)
		{
	...
	protected:
		...
		virtual const GvAttributesT& getAttrs() const override { return this->getAttrsT(AttrType::GRAPH); };
		virtual void printTo(printLineT pl) const override { this->printGraphTo(pl); }
	private:
		SubGraph(AbstractGraphT& g, strtype name, strtype label);
		bool cluster;
		virtual strtype getKeyword() const override;
	};
